...
% Last Modified by GUIDE v2.5 27-Dec-2017 15:23:11
...
handles.flag5=0; %load simulated (radiobox 2D selected by default)
...
BB=Segm_alg(XY,n0,prob,sigP1,1);

% alldata=handles.AA;
guidata(hObject,handles);
dlmwrite('segmentedata.txt',BB,'delimiter','\t')
...
rgb1=get(handles.simulate,'BackgroundColor');
...
% --- Executes when selected object is changed in select_data.
function select_data_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in select_data 
...
function select_dimensions_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in select_data 
% eventdata  structure with the following fields (see UIBUTTONGROUP)
%	EventName: string 'SelectionChanged' (read only)
%	OldValue: handle of the previously selected object or empty if none was selected
...
if hObject==handles.dosdim
    fg5=0; %2D trajectories
else
    fg5=1; %3D trajectories
...
% --- Executes on button press in Join_multitrajectories.
function Join_multitrajectories_Callback(hObject, eventdata, handles)
% hObject    handle to Join_multitrajectories (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
clear val
val=handles.flag5;
BB=Joincp(val);
if val==0
  dlmwrite('joined2Dcp.txt',BB,'delimiter','\t')  
elseif val==1
    dlmwrite('joined3Dcp.txt',BB,'delimiter','\t')
end
guidata(hObject,handles);
...
% handles    structure with handles and user data (see GUIDATA)
 ...
 msgbox('PLANT. SOFTWARE VERSION 1.2. A.Sosa-Costa & C. Manzo, Dec. 2017' )
